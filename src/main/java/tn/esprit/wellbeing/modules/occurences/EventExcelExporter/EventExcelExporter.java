package tn.esprit.wellbeing.modules.occurences.EventExcelExporter;
...
public class EventExcelExporter {
    ...
    private void writeHeaderLine() {
     ...
     //   createCell(row, 0, "Event ID", style);  
        ...
        createCell(row, 1, "THEME", style);       
        createCell(row, 2, "DSCRIPTION", style);       
    ...
    private void writeDataLines() {
        ...
        for (Event event : listEvents) {
            ...
            createCell(row, columnCount++, event.getTitle(), style);
            createCell(row, columnCount++, event.getTheme(), style);
            createCell(row, columnCount++, event.getDescription(), style);
            createCell(row, columnCount++, event.getStartDate().toString(), style);
            createCell(row, columnCount++, event.getEndDate().toString(), style);
